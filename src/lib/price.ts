...
type priceUpdate = { value: number; loading: boolean };
type priceUpdateFn = (updater: (prev: priceUpdate) => priceUpdate) => void;
export async function refreshPrice(update: priceUpdateFn) {
  try {
    update((v: priceUpdate) => {
